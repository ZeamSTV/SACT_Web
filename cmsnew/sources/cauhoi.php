...
switch($act){
	...
	case "man":
		gets();
		$template = "cauhoi/item";
		break;
	case "add":
		$template = "cauhoi/item_add";
		break;
	case "edit":
		get();
		$template = "cauhoi/item_add";
...
function get(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
	
	$sql = "select * from #_hoidap where  id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
	$item = $d->fetch_array();
}
...
function save(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
...
$data['luotlike'] =$_POST['luotlike'];
	...
	if($id){		
		...
		if($d->update($data))
				transfer("Cập nhật dữ liệu thành công", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
	}else{		
		...
		$d->setTable('hoidap');
		if($d->insert($data))
			redirect("index.php?com=cauhoi&act=man");
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
	}
}
...
function delete(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			redirect("index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
	}else transfer("Không nhận được dữ liệu", "index.php?com=cauhoi&act=man".$chuoi_noi_curpage);
}
