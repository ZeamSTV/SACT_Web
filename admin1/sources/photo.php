...
function save_chiase(){
	...
	if($id){		
		if($photo = upload_image("file", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF|SVG|svg', _upload_hinhanh,$file_name)){
			$data['photo'] = $photo;			
			$d->setTable('photo');
			$d->setWhere('id', $id);
...
foreach ($config["lang"] as $key => $value)  {
	...
	if($id){			
		if($photo = upload_image("file", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF|webp|WEBP|SVG|svg', _upload_hinhanh,$file_name)){
			$data['photo'] = $photo;
