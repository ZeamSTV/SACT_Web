<?php  if(!defined('_kiemtraweb') || _kiemtraweb!=$check_website) die('Eror');						
		...
		if(isset($_REQUEST['id'])){	//Chi tiết
			...
			$title_ttt = '<a href="xay-dung-nha-dan-dung/'.(($news['ten']!='')?q_bodautv($news['ten']):"-").'-'.$news['id'].'.htm" title="'.$news['ten'].'" class="transition_03s">'.$news['ten'].'</a>';
			$title_ttt1 = '<a href="xay-dung-nha-dan-dung.html" title="Xây dựng nhà dân dụng liên quan" class="transition_03s">Xây dựng nhà dân dụng liên quan</a>';
			...
			$title_ttt = '<a href="xay-dung-nha-dan-dung/'.(($news_cat['ten']!='')?q_bodautv($news_cat['ten']):"-").'-'.$news_cat['id'].'.html" title="'.$news_cat['ten'].'" class="transition_03s">'.$news_cat['ten'].'</a>';
			...
			$url='xay-dung-nha-dan-dung/'.(($news_cat['ten']!='')?q_bodautv($news_cat['ten']):"-").'-'.$news_cat['id'].'.html/';
			$maxR=12;
			$maxP=5;
			...
			$sql_tintuc = "select * from #_congtrinh where hienthi=1 and id_cat=12 order by stt asc, id desc";					
			$d->query($sql_tintuc);
			$news = $d->result_array();
			$title_ttt = '<a href="xay-dung-nha-dan-dung.html" title="Xây dựng nhà dân dụng" class="transition_03s">Xây dựng nhà dân dụng</a>';
					
			$count_sp = count($news); 
			
			$curPage = isset($_GET['p']) ? $_GET['p'] : 1;
			$url="xay-dung-nha-dan-dung.html/";
			$maxR=1111112;
			$maxP=5;
			$paging=paging_home($news, $url, $curPage, $maxR, $maxP);
			$news=$paging['source'];			
		...
		$title_bar = "Xây dựng nhà dân dụng - ".$title_bar;
?>