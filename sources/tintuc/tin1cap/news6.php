<?php  if(!defined('_kiemtraweb') || _kiemtraweb!=$check_website) die('Eror');						
		...
		if(isset($_REQUEST['id'])){	//Chi tiết
			...
			$title_ttt = '<a href="thi-cong/'.(($news['ten']!='')?q_bodautv($news['ten']):"-").'-'.$news['id'].'.htm" title="'.$news['ten'].'" class="transition_03s">'.$news['ten'].'</a>';
			$title_ttt1 = '<a href="thi-cong.html" title="Thi công liên quan" class="transition_03s">Thi công liên quan</a>';
			...
			$title_ttt = '<a href="thi-cong/'.(($news_cat['ten']!='')?q_bodautv($news_cat['ten']):"-").'-'.$news_cat['id'].'.html" title="'.$news_cat['ten'].'" class="transition_03s">'.$news_cat['ten'].'</a>';
			...
			$url='thi-cong/'.(($news_cat['ten']!='')?q_bodautv($news_cat['ten']):"-").'-'.$news_cat['id'].'.html/';
			$maxR=12;
			$maxP=5;
			...
			$sql_tintuc = "select * from #_congtrinh where hienthi=1 and id_cat=14 order by stt asc, id desc";					
			$d->query($sql_tintuc);
			$news = $d->result_array();
			$title_ttt = '<a href="thi-cong.html" title="Thi công" class="transition_03s">Thi công</a>';
